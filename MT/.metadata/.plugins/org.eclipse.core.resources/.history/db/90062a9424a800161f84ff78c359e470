...
public class B_29892_Allapps_MT2_RegistrationDetails_PLAYNWIN extends Action_Wrapper {
	...
	public void setup() throws Exception {
		...
		startTestCase("B_29892_Allapps_MT2_RegistrationDetails_PLAYNWIN");
		createAccessTokenForAllApp("playnwin");

		accessToken = AccessToken.getAccessToken("PLAYANDWIN", "new");
		...
		enableThread("B_29892_Allapps_MT2_RegistrationDetails_PLAYNWIN.jmx", threadGroup);
				
	}

	@Test(groups = {GroupName.RegFoundation, GroupName.Regression })
	public void story_B_29892_Allapps_MT2_RegistrationDetails_PLAYNWIN() throws Exception {

		//  *********************  Code Block for "Full Reg " Scenario ********************
		Log.info("Start Executing Jmeter Script");

		CallJmeterBuild("B_29892_Allapps_MT2_RegistrationDetails_PLAYNWIN.jmx");
		Log.info("JMX file Called");

		Log.info("Starting Assetion");
		Assert.assertEquals("false", xp.Xml_Parser("B_29892_Allapps_MT2_RegistrationDetails_PLAYNWIN.xml"));
		Log.info("Assetion Validation Completed for Full Reg");
		
		// ************************  Code Block for "Mini Reg " Scenario  **********************
		
		response = RF.createMiniRegUserAPI();
		writeInProprtyFile("EncMiniRegEmail", getEncryptedData("Access_Token_PLAYANDWIN",RF.getResponseValueAPI(response, ResponseData.Email)));
		...
		writeInProprtyFile("EncMiniRegGMT", getEncryptedData("Access_Token_PLAYANDWIN",RF.getResponseValueAPI(response, ResponseData.GlobalMemberToken)));
		
		writeInProprtyFile("MiniRegAddress_ID", RF.getResponseValueAPI(response, ResponseData.AddressId));
		copyAndRenameFile();
		
		enableThread("B_29892_Allapps_MT2_RegistrationDetails_PLAYNWIN.jmx", threadGroup1);
		...
		writeInProprtyFile("Encrypted_FBEmail", getEncryptedData("Access_Token_PLAYANDWIN",RF.getResponseValueAPI(response, ResponseData.Email)));
		...
		writeInProprtyFile("Encrypted_FBGMT", getEncryptedData("Access_Token_PLAYANDWIN",RF.getResponseValueAPI(response, ResponseData.GlobalMemberToken)));
		
		writeInProprtyFile("FBAddress_ID", RF.getResponseValueAPI(response, ResponseData.AddressId));
		copyAndRenameFile();
				
		enableThread("B_29892_Allapps_MT2_RegistrationDetails_PLAYNWIN.jmx", threadGroup2);
