...
public class B_35881_MT2_PCHApp_MetaActionBonusGameSupport_DaysInteract_LottoGame extends Action_Wrapper {
	...
	String bnsGameStart[]={"GameStart - FriendInvite Bonus"};
	
	RFTestPage RF = new RFTestPage();
	Jmeter_Xml_Parser xp = new Jmeter_Xml_Parser();
	String jmeterScriptName = "B_35881_MT2_PCHApp_MetaActionBonusGameSupport_DaysInteract_LottoGame";

	
	@BeforeClass
	public void setup() throws Exception {
		
		startTestCase("B_35881_MT2_PCHApp_MetaActionBonusGameSupport_DaysInteract_LottoGame");
		...
		joomla.PA.bonusActionArticle("Lotto Bonus DaysInteract","Days_Interact");
	...
	public void story_B_35881_MT2_PCHApp_MetaActionBonusGameSupport_DaysInteract_LottoGame() throws Exception {
		...
		Assert.assertTrue(DB.phpctrl_app_bonus_items(GMT,"get Bonus data").contains("{\"455\":{\"status\":3,\"data\":{\"buid\":\"53\"}}}"));
		//Repeat Visit Days Validation in "phpctrl_app_bonus_items_daily" table
		Assert.assertTrue(DB.phpctrl_app_bonus_items_daily(GMT,"get Bonus data").contains("\"repeatVisitDays\":1"));
		...
		/* This is not valid for lotto game, Because we are changing visit date in MT's DB but for lotto 
		 * response is coming from lotto's DB
		 
		//******* Thread for Bonus Submit using Lotto Game Start Day 2
		enableThread(jmeterScriptName+".jmx", bnsGameStart);
		Log.info("Start Executing Jmeter Script");
